...
static void copyShaderFromPacks() {
  ...
  fs::path mojang = fs::path(appData) / "Minecraft Education Edition" /
                    "games" / "com.mojang";
  fs::path packs = mojang / "resource_packs";
  if (!fs::exists(packs)) { Logger::log("no resource_packs folder"); return; }

  std::string cfg = readTextFile(mojang / "minecraftpe" /
  ...
  if (best.empty()) { Logger::log("active pack not found among packs"); return; }
